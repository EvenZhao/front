...
class PgCwdzz extends React.Component {
	constructor(props) {
	    super(props);
		this.state = {
			title: 'PgHome',
	...
	onTouchEnd(ev){
		...
		endY = ev.changedTouches[0].pageY;
		 ...
		 switch(direction) {
			...
			case 1:
					console.log("向上");
					if (danyeIndex==2) { //如果当前单页为最后一张，则回到第一张
						// danyeIndex=0
						return
					}else {
  ...
  onClickSpecial(e){
	...
	render(){
        ...
        <div style={{...styles.div,display:this.state.displayOne,height:this.state.height,position:this.state.upDown ? 'relative':'absolute',bottom:0}}>
					<img src={Dm.getUrl_img(`/img/v2/icons/CwdzzOne.png`)} height={devHeight} width={devWidth} style={{...styles.image}}/>
        </div>
				<div style={{...styles.div,display:this.state.displayTwo,height:this.state.height,position:this.state.upDown ? 'relative':'absolute',bottom:0}}>
					<img src={Dm.getUrl_img(`/img/v2/icons/CwdzzTwo.png`)} height={innerHeight} width={devWidth}  style={{...styles.image}}/>
				</div>
          ...
          <div style={{width:devWidth,textAlign:'center',marginBottom:devHeight*(22/667),}}>
            <span style={{color:'#f4f9fd'}}>注册网站即可申请免费体验直播或视频课程</span>
          </div>
					<input style={{...styles.inputText}} type="text" value={this.state.name} placeholder=" 姓名" onChange={this._onChangeName.bind(this)}/>
					<input style={{...styles.inputText}} type="text" value={this.state.position} placeholder=" 职位" onChange={this._onChangePosition.bind(this)}/>
					<input style={{...styles.inputText}} type="text" value={this.state.mobile} placeholder=" 手机" onChange={this._onChangeMobile.bind(this)}/>
					<input style={{...styles.inputText}} type="text" value={this.state.company} placeholder=" 公司" onChange={this._onChangeCompany.bind(this)}/>
					<input style={{...styles.inputText}} type="text" value={this.state.telephone} placeholder=" 电话" onChange={this._onChangeTelephone.bind(this)}/>
					<input style={{...styles.inputText}} type="text" value={this.state.email} placeholder=" 邮箱" onChange={this._onChangeEmail.bind(this)}/>
					<div style={{...styles.button}} onClick={this.onClickSpecial.bind(this)}>
						<span style={{color:'#f4f9fd'}}>提交</span>
					</div>
				</div>
					<img src={Dm.getUrl_img(`/img/v2/icons/CwdzzThree.png`)} height={devHeight} width={devWidth} style={{...styles.image}}/>
				</div>
				{
...
var styles = {
  ...
  zzc:{
	...
	from:{
		...
		width:devWidth,
		position:'absolute',
		top:window.innerHeight*(210/667),
		zIndex: 99
	}
};
export default PgCwdzz;
