...
class yyg extends React.Component {
  constructor(props) {
      super(props);
      ...
      this.clientType = 5;
      this.code = 'yyg';
      // this.uuid = '0bbb1b8b-69c6-11e6-bc9a-288023a21480';
      // this.source = '0';


    //  this.form='<form name="punchout_form" method="post" action="https://openapi.alipay.com/gateway.do?sign=BeA9lEetYI6USDqCKHm%2F9cnMtd8Nw%2B3R4z3JB2TRIdPFscmADhmbJQGGJ%2FaZXnKhPCwPPbIuAnLVdLDevgGn%2BWEjD0fzoK67FAY8S44i%2FdF5k%2FFZXJgwTVy8iOh%2FMGQWTL%2F1hAbgWVr%2FGNrijUMRg6ygVk6%2BMCIsuWvN2Fq02VJdKCbwyCu3wUxxOREK0EOLjgus9ZEH%2BCXARQO59dgMCmJkWUP6%2BykauFZCINvfaeYfLMLKiuLyjgkxXQU0erCJtZaei%2FDNLOj6C7DjUUFhDZH5ieoUJEUzKGMV%2F020ib%2FMoHIRgKHWZKYt7ZcF2geVqplzFxDPojRgMBhX8CCw6A%3D%3D&timestamp=2017-07-20+14%3A04%3A26&sign_type=RSA2&notify_url=http%3A%2F%2F112.64.196.194%3A8111%2Fpay%2Fadvnotify%2Falipay-notify&charset=UTF-8&app_id=2017071107716396&method=alipay.trade.wap.pay&version=1.0&alipay_sdk=alipay-sdk-java-dynamicVersionNo&format=json"> <input type="hidden" name="biz_content" value="{&quot;out_trade_no&quot;:&quot;W2017072014042524512&quot;,&quot;product_code&quot;:&quot;QUICK_WAP_PAY&quot;,&quot;subject&quot;:&quot;课程订单null&quot;,&quot;total_amount&quot;:&quot;100.00&quot;}"> <input type="submit" value="立即支付" style="display:none" > </form> <script>document.forms[0].submit();</script>'
  }
...
componentDidMount(){
  ...
  this.e_GetActivityUserInfo = EventCenter.on('GetActivityUserInfoDone',this._handleGetActivityUserInfo.bind(this));
  this.e_GetActivityVerifyCode = EventCenter.on('GetActivityVerifyCodeDone',this._handleGetActivityVerifyCode.bind(this));
...
componentWillMount(){
  ...
  Dispatcher.dispatch({
		actionType: 'GetActivityUserInfo',
		uuid:this.uuid,
    source:this.source,
    code:this.code,
	});
...
componentWillUnmount(){
  this.e_GetActivityUserInfo .remove()
  this.e_GetActivityVerifyCode.remove()
  this.e_PayForActivity.remove()
}

_handleGetActivityUserInfo(re){
  if(re.err){
    this.setState({
      errInfo:re.err,
    })
  ...
  if(re.result){
    this.setState({
      user:re.result||{},//isAttend:true表示已购买
      resource_id:re.result.resource_id || ''
    })
  }
}

_handleGetActivityVerifyCode(re){
...
_handlePayForActivity(re){
  ...
  if(re.result){
    ...
    this.setState({
      ...
      var form = <div dangerouslySetInnerHTML={{__html: f(stateFrom)}}
      				>
      				</div>
      ...
      this.setState({
        formHtml: form
          ...
          addScript.innerHTML = 'document.forms[0].submit();';
        	// addScript.setAttribute('javascript', '')
          setTimeout(() => {
...
_sendCode(){
  ...
  if(this.state.user.phone){//手机号存在,发送验证码
    this.setState({
      mobile:'',
    })
  }else {//手机号不存在，输入手机号
  ...
  if(this.state.isSend){
    this.setState({
      ...
      //发送验证码
      Dispatcher.dispatch({
        actionType:'GetActivityVerifyCode',
        uuid:this.uuid,
        code:this.code,
...
_buy(){
 ...
 if(this.state.user.phone){//手机号存在
      ...
      <div style={{...styles.container}}>
        <div style={styles.body_bg}>
          ...
          <div style={{...styles.content_bg}}>
            ...
            <div style={{fontSize:14,color:'#898989',marginTop:30/750*devWidth,}}>尊敬的<span style={{color:'#28aeb9'}}>{this.state.user.dispUser || ''}</span>用户，完成以下信息后，即可优惠购课</div>


            <div style={{visibility:this.state.user.isAttend ? 'hidden':'visible'}}>
              {this.state.user.phone ?
                <div style={styles.show_mobile}>
                  ...
                  <div style={styles.mobile_text}>{this.state.user.phone}</div>
                </div>
                :
              ...
              <div style={{...styles.activityInfo,display:this.state.user.isAttend ? 'block':'none',paddingTop:devHeight/1334*50,top:devWidth/750*320,height:devHeight/1334*160, lineHeight:'25px'}}>
