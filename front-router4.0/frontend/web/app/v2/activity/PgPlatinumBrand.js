...
var t
class PgPlatinumBrand extends React.Component {
  constructor(props) {
      super(props);
      this.wx_config_share_home = {
  				...
  				imgUrl: Dm.getUrl_img('/img/v2/activity/pla_share.jpg'),
  				type: 'link'
  		}
      this.state={
        name:'',
    ...
    return(
      ...
      <div style={{...styles.container}}>
        <div style={{width:devWidth,}}>
          <img src={Dm.getUrl_img(`/img/v2/activity/pla_top.jpg`)} width={devWidth} height={devWidth*(1637.5/375)} />
          <div style={{...styles.form_box}}>
            ...
            <div style={{width:devWidth-26,textAlign:'center',marginTop:19,paddingBottom:20,}}>
              <span style={{color:'#565554',fontSize:12,}}>了解更多欢迎详询:400-616-3899</span>
            </div>
            </div>
          </div>
          <img src={Dm.getUrl_img(`/img/v2/activity/pla_end.png`)} width={devWidth} height={devWidth*(91.5/375)} />
        </div>
        <div style={{...styles.show_box,display:this.state.isShow ?'block':'none'}}></div>
  ...
  onClickSubmit(){
    ...
    else if (!isCellPhoneAvailable(this.state.telephone)) {
      this.onIsShow('请输入正确手机号')
      return
    }
...
export default PgPlatinumBrand;
