...
class PgExam extends React.Component {
	constructor(props) {
		super(props);
		this.state = {
			exam: [],//所有试题集合
	...
	//获取章节测试题
	_handleGetExam(re) {
		console.log('pgExam--',re)
		// this.exam_index++
		this.setState({
			examDetail: re.results[0],
			exam: re.results[0].examInfos,
			enterpriseTask: re.results[0].enterpriseTask,
			catalogTitle: this.props.location.state ? this.props.location.state.catalogTitle : '',
			isLoading:false,
			...
			// examMulti: isMulti,
			// trueAnswer: [],
			// testCount:re.results[0].examInfos.length,
		}, () => {
	...
	_handlePostExam(re) {
		if(re) {			
			console.log(re)
			var correctNum = 0;
			this.state.examInfos.map((item, index) => {
				if(item.correct) {
					correctNum++
				}
			})
	...
	//“知道了” => 继续学习
	_popToOnlineDetail() {
		localStorage.setItem("CatalogIdx", this.props.location.state? this.props.location.state.catalogIdx : 0)
	...
	//查看解析
	goToAnalysis(){
		this.props.history.replace({pathname: `${__rootDir}/examDetail/${this.props.match.params.id}/${this.props.match.params.catalogId}`, query: null, hash: null, state: {uuid: this.state.uuid, catalogIdx: this.props.location.state? this.props.location.state.catalogIdx : 0}})
		// this.setState({
		// 	isAnalysis: true,
		// 	isShow: false
		// })
	...
	radomCommit(){
		var examInfos = this.state.exam.map((item, index) => {
			var correct = false;
			var true_answer = item.answers.sort().toString()
			var select_answer = [item.examContent[0].contentId].toString()
			if(true_answer == select_answer) {
				correct = true
			} else {
				correct = false
			...
			return{
				examId: item.examId,
				answer: [item.examContent[0].contentId],
				correct: correct
			}					
		})
		this.setState({
	...
	render(){
    	...
    	var exam = this.state.exam.map((item, index) => {
			...
			//遍历每一道题的选项
			var options = item.examContent.map((option,idx)=>{
				var choose = false;
				var optionIds = this.state.selectId[index] ? this.state.selectId[index] : []
				if(isMulti){
					if(optionIds.indexOf(option.contentId) > -1 ){
						choose = true;
					}
				}
				else{
					if(optionIds == option.contentId ){
						choose = true;
					}
				}
				return(
					<div key={idx} onClick={this._selected.bind(this,option.contentId,isMulti,index,item)}>
				...
				<div key={index} style={{paddingTop:20, height:this.devHeight-121,overflowY:'auto', display: index == this.state.displayNum ? 'block':'none'}}>
					<div ref={(ref)=>{this.refDom[index]=ref}}>
						<div className='clearfix' style={{fontSize:14,color:'#000'}}>
							...
							<span style={{color:'#000'}}>{item.examTitle}</span><span style={{marginLeft: 10, color: '#666'}}>({isMulti ? '多选题' : '单选题'})</span>
							</div>
						</div>
			...
			<div style={{...styles.alert,display:this.state.isShow ?'block':'none'}}>
					{
						...
						<div style={{...styles.alertFirstDiv2}}>
							<div style={{width:242,marginLeft:14,height:80,marginTop:10,position:'absolute',textAlign:'center'}}>
								<p style={{fontSize:17,color:'#030303',margin:5}}>
									{this.state.alertTitle}
								</p>
								<p style={{color:'#ff0000', fontSize:18,margin:5}}>{Math.round(parseInt(this.state.correctNum)*100/parseInt(this.state.exam.length))}%</p>
								<p style={{margin:5}}>答对<span style={{color:'#2196f3'}}>{this.state.correctNum}</span>题/共{this.state.exam.length}题</p>
							</div>
