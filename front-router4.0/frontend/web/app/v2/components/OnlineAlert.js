...
class OnlineAlert extends React.Component {
	constructor(props) {
    super(props);
    this.phone = '',
		this.state = {
  ...
  componentWillMount() {
	...
	render(){
		...
		var showAlert =(
			<div style={{...styles.white_alert,paddingTop:-1}}>
				...
				<div style={{ color: '#333',fontSize:Fnt_Medium}}>您尚未获得此视频观看权限</div>
				<div style={styles.alert_bottom}>
					<div style={{display: 'flex', flex:1, alignItems: 'center', flexDirection: 'column', justifyContent: 'center', lineHeight: '42px',borderRight:'solid 1px #d4d4d4',fontSize:Fnt_Medium,color:Common.Activity_Text}} onClick={this._useDiscount.bind(this)}>使用体验券</div>
					<div style={{display: 'flex',flex:1, alignItems: 'center', flexDirection: 'column', justifyContent: 'center', lineHeight: '42px',fontSize:Fnt_Medium,color:Common.Gray}}  onClick={this._applyVoucher.bind(this)}>申请免费体验</div>
				</div>
			</div>
		)
		...
		var couponShow = (
			<div style={{...styles.white_alert,paddingTop:-1}}>
				...
				<div style={styles.alert_bottom}>
					<div style={{display: 'flex', flex:1, alignItems: 'center', flexDirection: 'column', justifyContent: 'center', lineHeight: '42px',borderRight:'solid 1px #d4d4d4',fontSize:Fnt_Medium,color:Common.Activity_Text}} onClick={this._useDiscount.bind(this)}>使用优惠券
					</div>
    ...
    return(
      <div style={{display: this.props.isShow ? 'block' : 'none'}}>
				...
				{
					this.props.resource_count > 0 ?
					addShow
					:
					couponShow
				}
				{/*
					this.props.resource_count == null || this.props.resource_count =='' || this.props.resource_count == 0 ?
					couponShow
					:
					null
				*/}
