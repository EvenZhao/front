...
class AnswerNoticeList extends React.Component {
	constructor(props) {
	    super(props);
			this.dataSkip = 0
	...
	componentDidMount() {
		...
		EventCenter.emit("SET_TITLE",'铂略财课-回答通知')
 ...
 Analysis_body(content,firstArry,keyArry,strArry){
    ...
    var list = this.state.data.map((item,index)=>{
		 ...
		 var new_sendTime = someDay(item.send_time);//新增action日期格式不同
		 var context //内容
       ...
       return(
				 <div key={index}>
  				 ...
  				 <div style={{...styles.div_box,backgroundColor:BgColor}}  onClick={this.gotToDetail.bind(this,jumpUrl,item.id,item.type,item.action)}>
  					 ...
  					 <div style={{...styles.TitleDiv}} dangerouslySetInnerHTML={{__html: title}}>

  					 </div>
						 <div style={{...styles.note_text,...styles.LineClamp}} >
						 	{content.answererNickname}：{this.removeHtmlTag(content.answerContent)}
						 </div>
						 <div style={{...styles.LineClamp,...styles.reply_box,backgroundColor:boxBgColor}}>
						 	{this.removeHtmlTag(content.questionTitle)}
						 </div>
...
export default AnswerNoticeList;
